...
export const Card = ({ children, className = '', span = '', onClick }) => {
  ...
  return (
    <div className={`relative h-full w-full ${span}`}>
      <div
        onClick={onClick}
        className={`glass-panel glow-primary rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 flex flex-col justify-between h-full transition-all duration-300 ease-out
          ${isClickable
            ? 'hover:shadow-xl hover:border-indigo-500/30 dark:hover:border-indigo-400/40 hover:-translate-y-1 hover:scale-[1.015] cursor-pointer'
            : 'cursor-default'
          } 
